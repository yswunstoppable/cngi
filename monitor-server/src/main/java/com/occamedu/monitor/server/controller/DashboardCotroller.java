...
import com.occamedu.monitor.server.dto.ChartInfo;
import com.occamedu.monitor.server.dto.NetIoStateDto;
import com.occamedu.monitor.server.entity.*;
import com.occamedu.monitor.server.service.*;
import com.occamedu.monitor.server.util.DateUtil;
import com.occamedu.monitor.server.util.FormatUtil;
import com.occamedu.monitor.server.util.PageUtil;
...
public class DashboardCotroller {
    ...
    @Resource
    TempStateService tempStateService;
    @Resource
    HumidityStateService humidityStateService;
    @Resource
    ...
    public String systemInfoList(SystemInfo systemInfo, Model model, HttpServletRequest request) {
        ...
        if (request.getParameter(StaticKeys.DISK) != null) {
            return "host/disk";
        }
        if (request.getParameter(StaticKeys.FLOW) != null) {
            return "host/flow";
        }
        if (request.getParameter(StaticKeys.CARRIAGE) != null) {
            return "host/carriage";
        }
        if (request.getParameter(StaticKeys.TEMP) != null) {
            return "host/temp";
        }
        if (request.getParameter(StaticKeys.HUMIDITY) != null) {
            return "host/humidity";
        } else {
            return "host/list";
        }
    }
    ...
    public String hostChart(Model model, HttpServletRequest request) {
        ...
        try {
            ...
            List<TempState> tempStateList = tempStateService.selectAllByParams(params);
            model.addAttribute("tempStateList", JSONUtil.parseArray(tempStateList));
            List<HumidityState> humidityStateList = humidityStateService.selectAllByParams(params);
            model.addAttribute("humidityStateList", JSONUtil.parseArray(humidityStateList));
        ...
        if (request.getParameter(StaticKeys.FLOW) != null) {
            return "host/flowChart";
        }
        if (request.getParameter(StaticKeys.CARRIAGE) != null) {
            return "host/carriageChart";
        }
        if (request.getParameter(StaticKeys.TEMP) != null) {
            return "host/tempChart";
        }
        if (request.getParameter(StaticKeys.HUMIDITY) != null) {
            return "host/humidityChart";
        } else {
