<template>
  <div>
    <a-card :bordered="false">
      <BoundarySearchForm @search="(data)=>this.search(data)" />
    ...
    >
      <a-form-model
        ...
        :wrapper-col="{ span: 20 }"
      >
        <a-form-model-item label="防火墙IP" prop="ipAddress">
          <a-input v-model="editForm.form.ipAddress" placeholder="请输入防火墙IP" />
        </a-form-model-item>
        <a-form-model-item label="用户名" prop="username">
          <a-input v-model="editForm.form.username" placeholder="请输入用户名" />
        </a-form-model-item>
        <a-form-model-item label="密码" prop="password">
          <a-input v-model="editForm.form.password" placeholder="请输入密码" />
        </a-form-model-item>
...
import BoundarySearchForm from './components/BoundarySearchForm'
import cloneDeep from 'lodash.clonedeep'
import { parseFormData } from '@/utils'
import { delBoundary, editBoundary, judgeBoundaryField, listBoundary } from '@/api/center'
...
export default {
  ...
  data () {
    const validateIpAddress = (rule, value, callback) => {
      if (value === '') {
        callback(new Error('请输入防火墙IP地址'))
        return
      }
      judgeBoundaryField(this.editForm.form.id, 'ip_address', value).then((response) => {
        const { data } = response
        if (data === 'fail') {
          callback(new Error('防火墙IP已存在'))
        }
        callback()
      })
    }
    ...
    return {
      ...
      table: {
        ...
        loadData: parameter => {
          ...
          return listBoundary(requestParameters).then(res => {
